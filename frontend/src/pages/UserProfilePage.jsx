...
export default function UserProfilePage() {
    ...
    const { user, addresses, wishlist, preferences, isAuthenticated } = useSelector((state) => state.auth);
    ...
    if (!isAuthenticated || !user) {
        return (
            <div className="max-w-md mx-auto my-20 bg-white border border-slate-200/80 rounded-2xl p-8 text-center space-y-4 shadow-sm">
                <div className="w-12 h-12 rounded-full bg-slate-100 text-slate-500 flex items-center justify-center mx-auto">
                    <User className="w-6 h-6" />
                </div>
                <h2 className="text-xl font-extrabold text-[#0F172A] font-heading">Logged Out</h2>
                <p className="text-xs text-slate-500">You have been signed out of your ShopGround account.</p>
                <Button onClick={() => window.location.reload()} className="gradient-btn-primary text-xs w-full">
                    Log In to Account
                </Button>
            </div>
        );
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
            ...
            <div className="relative bg-white border border-slate-200/80 rounded-2xl p-6 sm:p-8 shadow-xs overflow-hidden linear-shimmer-card">
                    ...
                    <div className="flex flex-col sm:flex-row items-center gap-5 text-center sm:text-left">
                        <div className="relative group">
                            <img
                                src={user?.avatar}
                                alt={user?.name}
                                className="w-20 h-20 rounded-full object-cover border-4 border-[#FAFAFC] shadow-md"
                            />
                            <div className="absolute bottom-0 right-0 p-1.5 bg-[#4F46E5] text-white rounded-full shadow-xs cursor-pointer hover:scale-110 transition-transform">
                        ...
                        <div className="space-y-1">
                            <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2">
                                <h1 className="text-2xl font-extrabold text-[#0F172A] font-heading">{user?.name}</h1>
                                <Badge className="bg-[#4F46E5] text-white text-xs">{user?.role}</Badge>
                            </div>
                            <p className="text-xs text-slate-500 flex flex-wrap items-center justify-center sm:justify-start gap-2 font-sans">
                                <span className="flex items-center gap-1"><Mail className="w-3.5 h-3.5 text-[#4F46E5]" /> {user?.email}</span>
                                <span className="text-slate-300">•</span>
                                <span className="flex items-center gap-1"><Phone className="w-3.5 h-3.5 text-[#4F46E5]" /> {user?.phone}</span>
                            </p>
                            <p className="text-[11px] text-slate-400">Member since {user?.memberSince}</p>
                        </div>
                    </div>

                    {/* Right E-Commerce Account Quick Stats & Logout Button */}
                    <div className="flex flex-col sm:flex-row items-center gap-4 w-full md:w-auto">
                        <div className="grid grid-cols-3 gap-3 border-t md:border-t-0 md:border-l border-slate-200/80 pt-4 md:pt-0 md:pl-6 w-full md:w-auto">
                            <div className="bg-[#FAFAFC] p-3 rounded-xl text-center border border-slate-200/60">
                            ...
                            <div className="bg-[#FAFAFC] p-3 rounded-xl text-center border border-slate-200/60">
                                ...
                                <span className="text-xl font-extrabold text-[#4F46E5]">${totalSpent.toFixed(2)}</span>
                            </div>
                            <div className="bg-[#FAFAFC] p-3 rounded-xl text-center border border-slate-200/60">
                        ...
                        {/* Top Logout Trigger Button */}
                        <Button
                            onClick={handleLogout}
                            variant="outline"
                            className="w-full sm:w-auto border-rose-200 text-rose-600 hover:bg-rose-600 hover:text-white font-bold text-xs gap-1.5 transition-colors shadow-2xs h-10 px-4"
                        >
                ...
                <div className="space-y-6">
                        ...
                        <div className="flex items-center gap-2 bg-white p-1 rounded-lg border border-[#E5E7EB]">
                            <button
                                ...
                                className={`px-3 py-1 rounded text-xs font-semibold ${
                                    orderFilter === 'all' ? 'bg-[#4F46E5] text-white' : 'text-slate-600 hover:text-slate-900'
                                }`}
                            >
                            ...
                            <button
                                ...
                                className={`px-3 py-1 rounded text-xs font-semibold ${
                                    orderFilter === 'Processing' ? 'bg-[#4F46E5] text-white' : 'text-slate-600 hover:text-slate-900'
                                }`}
                            >
                                Processing
                            </button>
                            <button
                                ...
                                className={`px-3 py-1 rounded text-xs font-semibold ${
                                    orderFilter === 'Delivered' ? 'bg-[#4F46E5] text-white' : 'text-slate-600 hover:text-slate-900'
                                }`}
                            >
                                Delivered
                            </button>
                    ...
                    {filteredOrders.length > 0 ? (
                        <div className="space-y-4">
                            {filteredOrders.map((order) => (
                                <Card key={order.id} className="bg-white border-[#E5E7EB] shadow-xs overflow-hidden">
                                    {/* Order Top Bar */}
                                    <div className="bg-[#FAFAFC] border-b border-[#E5E7EB] py-3.5 px-6 flex flex-wrap items-center justify-between gap-4">
                                        ...
                                        <div className="flex items-center gap-3">
                                            <Badge
                                                ...
                                                className={order.status === 'Processing' ? 'bg-[#4F46E5] text-white' : ''}
                                            >
                                                {order.status}
                                            </Badge>
                                    ...
                                    <CardContent className="p-6 space-y-4">
                                            ...
                                            {order.items.map((item, idx) => (
                                                <div key={idx} className="flex items-center justify-between text-xs border-b border-slate-100 pb-2">
                                                    <div className="flex items-center gap-2">
                                                        <span className="w-5 h-5 rounded bg-[#FAFAFC] text-[#4F46E5] font-bold flex items-center justify-center text-[10px]">
                                                            {item.qty}x
                                                        </span>
                                        ...
                                        <div className="bg-slate-50 p-3 rounded-lg border border-slate-200 flex items-center justify-between text-xs">
                                            ...
                                            <Button
                                                ...
                                                size="sm"
                                                variant="outline"
                                                className="text-xs h-8 gap-1.5 border-[#E5E7EB] bg-white text-[#4F46E5] hover:bg-[#FAFAFC]"
                                            >
                        ...
                        <div className="text-center py-12 bg-white border border-[#E5E7EB] rounded-xl p-6">
                            ...
                            <Button onClick={() => navigate('/')} className="mt-4 bg-[#4F46E5] text-white text-xs">
                                Start Shopping
                            </Button>
                        </div>
                    )}
                    ...
                    <CardContent>
                        <form onSubmit={handleSaveProfile} className="space-y-4 max-w-xl">
                            ...
                            <div className="pt-2 flex items-center gap-3">
                                <Button type="submit" className="bg-[#4F46E5] hover:bg-[#4f5bc4] text-white text-xs font-semibold px-6 py-2">
                                    Save Changes
                                </Button>
                                {saveSuccess && (
                ...
                <div className="space-y-6">
                    <div className="flex items-center justify-between">
                        ...
                        <Button
                            ...
                            size="sm"
                            className="bg-[#4F46E5] text-white text-xs font-semibold gap-1.5"
                        >
                            ...
                            <div className="bg-white border border-[#E5E7EB] rounded-2xl p-6 w-full max-w-md space-y-4">
                                ...
                                <form onSubmit={handleAddAddressSubmit} className="space-y-3 text-xs">
                                    ...
                                    <div className="pt-3 flex justify-end gap-2">
                                        ...
                                        <Button type="submit" className="bg-[#4F46E5] text-white text-xs font-semibold">
                                            Save Location
                                        </Button>
                                    </div>
                                </form>
                            ...
                            {wishlist.map((item) => (
                                <Card key={item.id} className="bg-white border-[#E5E7EB] p-4 flex gap-4 items-center">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-xs font-extrabold text-[#4F46E5]">${item.price.toFixed(2)}</p>
                                        <div className="flex gap-2 mt-2">
                                            <Button
                                                size="sm"
                                                onClick={() => dispatch(addToCart(item))}
                                                className="bg-[#4F46E5] text-white text-[11px] h-7 px-2.5 gap-1"
                                            >
            ...
            {activeTab === 'settings' && (
                <Card className="bg-white border-[#E5E7EB]">
                    ...
                    <CardContent className="space-y-6 max-w-xl">
                        ...
                        <div className="space-y-4">
                            <h4 className="text-xs font-bold text-slate-800 uppercase tracking-wider">Security & Account Action</h4>
                            <div className="flex justify-between items-center bg-[#FAFAFC] p-3.5 rounded-xl border border-slate-200">
                                <div>
                                    <p className="text-xs font-semibold text-slate-800">Sign Out of Account</p>
                                    <p className="text-[11px] text-slate-400">End active session on this device.</p>
                                </div>
                                <Button
                                    onClick={handleLogout}
                                    variant="outline"
                                    size="sm"
                                    className="text-xs border-rose-200 text-rose-600 hover:bg-rose-600 hover:text-white font-bold gap-1"
                                >
